...
def get_config_parameters(item_subtype):
    if ItemSubType.KNIFE == item_subtype:
        ...
        config_parameters = ConfigParameters(page_control_var_name='LAST_PROCESSED_PAGE_SHOTGUN',
                                             item_query='&category_730_Type[]=tag_CSGO_Type_Shotgun',
                                             item_type='weapon', item_subtype='shotgun')
    elif ItemSubType.SNIPER == item_subtype:
        config_parameters = ConfigParameters(page_control_var_name='LAST_PROCESSED_PAGE_SNIPER',
                                             item_query='&category_730_Type[]=tag_CSGO_Type_SniperRifle',
                                             item_type='weapon', item_subtype='sniper')
...
def crawl(config_parameters: ConfigParameters):
    ...
    steam_item_crawler = SteamItemCrawler(os.getenv('STEAM_API_KEY'), int(os.getenv('BATCH_SIZE')),
                                          os.getenv('DATABASE_URL'), '.env')
    steam_item_crawler.enrich_data(config_parameters.item_query, last_processed_page,
                                   config_parameters.page_control_var_name,
